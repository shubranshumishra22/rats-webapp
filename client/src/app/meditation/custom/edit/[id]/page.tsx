// client/src/app/meditation/custom/edit/[id]/page.tsx
'use client';
...
// --- MAIN PAGE COMPONENT ---
export default function EditCustomMeditationPage({ params }: { params: { id: string } }) {
  const [customMeditation, setCustomMeditation] = useState<CustomMeditation>({
    _id: '',
    name: '',
    introVoice: 'female',
    breathworkType: 'deep',
  ...
  // Fetch custom meditation
  const fetchCustomMeditation = useCallback(async () => {
    try {
      setIsLoading(true);
      
      const response = await axios.get(
        `http://localhost:5001/api/meditation/custom/${params.id}`,
        getAuthHeaders()
      );
      
      setCustomMeditation(response.data);
    } catch (error) {
      console.error('Failed to fetch custom meditation:', error);
  ...
  const handleSubmit = async () => {
    try {
      ...
      console.error('Failed to update custom meditation:', error);
      alert('Failed to update custom meditation. Please try again.');
      setIsSaving(false);
    }
  };
  ...
  useEffect(() => {
    ...
    try {
      ...
      fetchCustomMeditation();
    } catch (error) {
      console.error('Error decoding token:', error);
      localStorage.removeItem('rats_token');
      router.push('/login');
    }
  }, [router, fetchCustomMeditation]);
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      case 1:
        return (
          <div>
            <h2 className="text-xl font-semibold mb-4">Edit Meditation Name</h2>
            <input
              type="text"
              ...
              {introVoiceOptions.map((option) => (
                <OptionCard
                  ...
                  onClick={() => setCustomMeditation({ 
                    ...customMeditation, 
                    introVoice: option.id as 'male' | 'female' | 'none' 
                  })}
                />
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {breathworkOptions.map((option) => (
                <OptionCard
                  ...
                  onClick={() => setCustomMeditation({ 
                    ...customMeditation, 
                    breathworkType: option.id as 'box' | '4-7-8' | 'deep' | 'alternate-nostril' | 'none' 
                  })}
                />
              ))}
            </div>
      ...
      case 6:
        return (
          <div>
            <h2 className="text-xl font-semibold mb-4">Review Your Changes</h2>
            <div className="bg-white p-6 rounded-lg border border-gray-200">
              <h3 className="font-semibold text-lg mb-4">{customMeditation.name}</h3>
      ...
      <div className="flex justify-between mt-8">
        ...
        {currentStep < 6 ? (
          ...
          <button
            ...
            disabled={isSaving}
            className="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 ml-auto disabled:bg-gray-400"
          >
  ...
  if (isLoading) {
    return <div className="flex justify-center items-center min-h-screen">Loading meditation details...</div>;
  }
      ...
      <div className="min-h-screen bg-gray-50 pb-10">
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-3xl font-bold">Edit Custom Meditation</h1>
            <Link 
          ...
          {saveSuccess && (
            <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-6 flex items-center">
              ...
              <span>Meditation updated successfully! Redirecting...</span>
            </div>
          )}
